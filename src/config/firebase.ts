...
class FirebaseService {
  ...
  private db: any;
  private rtdb: any;
  private messaging: any; // Added for FCM
  ...
  private constructor() {
    try {
      ...
      // Initialize Firestore, Realtime Database, and Messaging
      this.db = getFirestore(app);
      this.rtdb = getDatabase(app);
      this.messaging = getMessaging(app); // Initialize FCM

      console.log("Firebase initialized successfully with FCM support");
    } catch (error) {
      console.error("Error initializing Firebase:", error);
      throw error;
    }
  ...
  // New method for sending FCM notifications
...
// Export the Firebase service instance and admin
export const firebaseService = FirebaseService.getInstance();
export { admin };
